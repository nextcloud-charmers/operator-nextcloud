...
# DISABLED: removing config files and replacing with backup breaks a new deployment of Nextcloud
# # File directory
...
# DISABLED: using pg_restore with clean instead
# #
# # Restore database
# #
...
# if [ "${databaseSystem,,}" = "mysql" ] || [ "${databaseSystem,,}" = "mariadb" ]; then
#     mysql -h "${dbHost}" -u "${dbUser}" -p"${dbPassword}" -e "DROP DATABASE ${nextcloudDatabase}"
...
#         mysql -h "${dbHost}" -u "${dbUser}" -p"${dbPassword}" -e "CREATE DATABASE ${nextcloudDatabase}"
#     else
#         # Database from the backup uses UTF8 with multibyte support (e.g. for emoijs in filenames)
#         mysql -h "${dbHost}" -u "${dbUser}" -p"${dbPassword}" -e "CREATE DATABASE ${nextcloudDatabase} CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci"
#     fi
# elif [ "${databaseSystem,,}" = "postgresql" ] || [ "${databaseSystem,,}" = "pgsql" ]; then
...
if [ "${databaseSystem,,}" = "mysql" ] || [ "${databaseSystem,,}" = "mariadb" ]; then
	mysql -h "${dbHost}" -u "${dbUser}" -p"${dbPassword}" "${nextcloudDatabase}" < "${currentRestoreDir}/${fileNameBackupDb}"
elif [ "${databaseSystem,,}" = "postgresql" ] || [ "${databaseSystem,,}" = "pgsql" ]; then
	PGPASSWORD="${dbPassword}" pg_restore --clean -d "${nextcloudDatabase}" -h "${dbHost}" -U "${dbUser}" -n public "${currentRestoreDir}/${fileNameBackupDb}"
fi
